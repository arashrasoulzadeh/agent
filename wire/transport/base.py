...
`service/rooms.py`, `wire/events.py`, and `wire/routes.py` — the
...
gRPC transport later a matter of writing one new adapter class next to
`websocket.py` and one new accept-loop in `wire/app.py` (or a sibling
module) — zero changes to `rooms.py`/`events.py`/`routes.py`.
...
class Transport(ABC):
    ...
    async def send(self, message: dict[str, Any]) -> None:
        ...
        (a closed connection, etc.) — callers (see wire/events.py) treat
        that as "this client is gone" and drop it, they don't swallow
        the error silently."""
