...
This file — along with wire/transport/websocket.py's WebSocketTransport
— is the *only* WebSocket-specific code in the delivery path. `routes.py`,
`service/rooms.py`, and `events.py` only ever see a `Transport`; a REST or
gRPC adapter would be a sibling module here (its own accept loop, its own
...
shutdown (SIGINT/SIGTERM) — see wire/lifecycle.py.
"""
...
from tool import LIFECYCLE_MODULES
from wire import lifecycle
from wire.config import HOST, PORT
from wire.protocol import ProtocolError, Request, error_response, response
from wire.routes import ROUTES
from wire.transport.websocket import WebSocketTransport

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(name)s %(message)s")
logger = logging.getLogger("wire.app")
