...
`agent/` has no business knowing whether it's driven by a websocket
server, a test, or nothing at all — so it reports through this narrow
interface instead of importing a renderer directly. `service/rooms.py`
implements one that broadcasts protocol events; `NullSink` is the default,
so `agent/` stays fully usable standalone (e.g. in tests) with no
sink at all.
"""
