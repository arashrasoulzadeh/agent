...
Orchestrates three stages (agent/stage.py, agent/stages.py):
...
constructor, and an optional `events` bus (agent/events.py) reports
...
from agent.events import StageEventBus
from agent.stage import Pipeline, PipelineContext, Turn
from agent.stages import STAGE_FACTORIES
...
class ProjectPipeline:
    def __init__(
        ...
        # this package doesn't import llm/ or tool/ to build any of that
        # itself, so it stays reusable and testable without either. The
        # caller (service/rooms.py, in the real app) builds the concrete
        # objects — including calling llm.get_llm(...) and handing in
        # tool.AGENT_TOOLS — and passes them in. Missing ones surface as a
        # clear error the first time they're needed
        ...
        for name in self.config.stages:
            ...
            if factory is None:
                raise ValueError(
                    ...
                    "with agent.stages.register_stage()"
                )
    ...
    def _run_sync(self, pipeline: Pipeline, turn: Turn) -> Turn:
        ...
        Meant to be called from the one worker thread service/rooms.py
        ...
        code with no loop of its own — see agent/stage.py's module
