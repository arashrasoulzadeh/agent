...
stages (agent/stages.py) wrap synchronous work: collection is a
filesystem walk, analysis and synthesis are synchronous LangChain calls.
They run inside the one worker thread service/rooms.py already dedicates
to a turn (via `asyncio.run()` inside that thread — see
ProjectPipeline.ask() in agent/__init__.py), so blocking here does not
...
class PipelineContext:
    ...
    lifecycle is reported to (see agent/events.py). Both are opt-in —
