...
JSON strings: encoding is a `Transport`'s job (see server/transport.py),
not this module's — a future gRPC transport wants protobuf, not JSON.
Only `Request.parse()` decodes JSON, because *decoding the request body*
is the one wire-format detail that hasn't moved behind Transport yet
(each transport adapter parses its own incoming frames before handing a
plain dict to `server/routes.py` — WebSocketTransport's caller,
server/app.py, does that with `Request.parse`).
